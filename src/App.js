...
// POST REVIEW FOR COMMENT
import PostReview from './components/mainPages/components/activityPosts/postReview';
const App = () => {
	return (
		<div className='app'>
			<BrowserRouter>
				<AuthContextProvider>
					<AuthContext.Consumer>
						{(value) => {
							return (
								<>
									{!value.state && <MainNavbar />}
									{!!value.state && <MainNavbarTwo />}
									<Switch>
										...
										{/*POSTREVIEW FOR COMMENT*/}
										<Route path='/post/:postId' component={PostReview} />
									</Switch>
									<Footer />
								</>
							);
